...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t1 = space();
    			input = element("input");
    			add_location(label, file, 191, 2, 5628);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1");
    			attr_dev(input, "max", "1440");
    			add_location(input, file, 192, 2, 5661);
    			attr_dev(div, "class", "overlay");
    			add_location(div, file, 190, 4, 5603);
    			add_location(main, file, 189, 0, 5591);
    		},
    ...
    function colorScale(ratio) {
    	const scaleColor = d3.scaleLinear().domain([0, 1]).range(['teal', 'red']);
    	return scaleColor(ratio);
    }

    function scaleRadiusTrafficVolume(traffic, maxTraffic = 500) {
    	const scaleRadius = d3.scaleLinear().domain([0, 1, maxTraffic]).range([0, 3, 20]);
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	mapboxgl.accessToken = "pk.eyJ1IjoiY2hhcmxlc3p3IiwiYSI6ImNsc3A3d2xwMTBuamMyam1qNHdsdG03YzYifQ.Tm7NH1JXCpUCICVS8dCOhg";
    	...
    	map.on("load", () => {
    		...
    		map.addLayer({
    			...
    			type: "line",
    			source: "boston_route",
    			paint: { "line-color": "#009933", "line-width": 3 }
    		});
    	...
    	function create_station_markers(station_data) {
    		station_markers = marker_container.selectAll("circle").data(station_data).enter().append("circle").attr("r", 5).style("fill", "#008080").attr("stroke", "#008080").attr("stroke-width", 1).attr("fill-opacity", 0.4).attr("name", function (d) {
    			return d["name"];
    		});
